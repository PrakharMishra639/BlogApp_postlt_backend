...
const updatePostCategory = async (req, res, next) => {
  ...
  const postCategory =await PostCategories.findByIdAndUpdate(
    ...
    if(!postCategory){
     const error = new Error("Catgeory was not found");
     return next(error);
    }
    return res.json(postCategory);
    }
...
await Post.updateMany(
    {categories : {$in: [categoryId]}}, { $pull: {categoryId}}
);
await PostCategories.deleteOne({ _id :categoryId});
res.send({
    message: "PostCcategory is successfully deleted",
});
}
catch(error){
 next(error);   
...
export { createPostCategory , getSingleCategory,getAllPostCategories , updatePostCategory, deletePostCategory};